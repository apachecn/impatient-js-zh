...
`assert.equal()`是一个方法调用（对象是`assert`，方法是`.equal()`），有两个参数：实际结果和预期结果。它是 Node.js 断言 API 的一部分，本书后面的[将对此进行解释](docs/10)[。](docs/10)

打印日志到[浏览器的控制台](/docs/7?id=_531。浏览器控制台)或 Node.js：
```javascript
// 将值标准输出打印（另一种方法调用）
console.log("Hello!");
```
...
上面的代码包含以下箭头函数（关于*表达式*和*语句块*等术语将在在[本章后面解释](docs/8?id=_64。语句与表达)）:
...
*   [通过对象解构分配](docs/41?id=_3443。语法陷阱：通过对象解构分配)
*   [从箭头函数](/docs/28?id=_2333。箭头函数返回对象字面值)
