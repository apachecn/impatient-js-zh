...
- 该图区分了类 `Object` 的对象和实例。 `Object` 的每个实例也是一个对象，但反之亦然。但是，实际上你在实践中遇到的所有对象都是 `Object` 的实例。例如，通过对象字面值创建的对象。关于该主题的更多细节在[26.4.3.4 对象并非 `Object` 的实例](/docs/32.md#264子类)中进行解释。
